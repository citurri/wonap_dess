...
package com.develop.android.wonap.common;
...
public class Utils {
    ...
    public static String[] getLocality(Context context) {
        ...
        if (Utils.isConn(context)) {
            Geocoder gcd = new Geocoder(context, Locale.getDefault());
            mLatestLocation = Utils.getLocation(context);
            //Log.v("GPS:", context.toString());
            if(mLatestLocation != null) {
                try {
                    addresses = gcd.getFromLocation(mLatestLocation.latitude, mLatestLocation.longitude, 1);
                    if (locality.length > 0) {
                        locality[0] = addresses.get(0).getLocality();
                        locality[1] = addresses.get(0).getCountryName();
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
    ...
    public static Boolean WonapLogin(Context context){
        Boolean login = false;

        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(context);
        Integer sesion_usuario = preferences.getInt("session_usuario", 0);
        if (sesion_usuario > 0) login = true;


        return login;
    }

    public static Double formatDistanceBetweenMetros(LatLng point1, LatLng point2) {
        if (point1 == null || point2 == null) {
            return null;
        }
